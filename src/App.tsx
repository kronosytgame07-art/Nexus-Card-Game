...
import { activateSupportCard, activateUnitEffect, aiDrawPhase, aiEndPhase, aiMainPhase, aiPrepareBattlePlan, aiResolveOneAttack, declareAttack, evolveUnit, MAX_FIELD_UNITS, MAX_SUPPORT, newGame, playCard } from './engine/engine';
    ...
    <aside className="app-sidebar">
      ...
      {navItems.map(({ section, active }) => (
        <Link key={section.label} to={section.to} className={'nav-link' + (active ? ' active' : '')}>
          <span className="nav-link-icon">{section.icon}</span>{section.label}
        </Link>
      ))}
    </aside>
    <main>{children}</main>
...
function CollectionHub() {
  ...
  return <>
    <div className="subtabs">
      <Link to="/collection" className={'subtab' + (tab === 'cartes' ? ' active' : '')}>Cartes</Link>
...
function Combat() {
  ...
  const enemyHasTaunt = match.enemy.field.some((unit) => unit.taunt && unit.stunnedTurns === 0); const activePlayerUnit = inspectedUnit ? match.player.field.find((unit) => unit.instanceId === inspectedUnit) : undefined;
...
function SocialHub() {
  ...
  return <>
    <div className="subtabs">
      <Link to="/profil" className={'subtab' + (tab === 'profil' ? ' active' : '')}>Profil</Link>
...
function Shop() {
  ...
  return (
    <section>
      ...
      {reveal && (() => { const staggerDelay = reveal.cards.length > BOOSTER_PULL_COUNT ? 0.04 : 0.18; return <div className="pile-modal" role="dialog" aria-modal="true" onClick={closeReveal}><div className="pile-modal-content" onClick={(e) => e.stopPropagation()}><header><h3>Booster {reveal.faction} ({reveal.cards.length} cartes)</h3><button onClick={closeReveal}>×</button></header><div className="pile-grid">{reveal.cards.map((card, i) => <motion.div key={`${card.id}-${i}`} initial={{ opacity: 0, y: 20, rotateY: 90 }} animate={{ opacity: 1, y: 0, rotateY: 0 }} transition={{ delay: i * staggerDelay, duration: 0.4, ease: 'easeOut' }}><CardView card={card} badge={reveal.isNew[i] ? 'Nouveau' : 'Doublon'} /></motion.div>)}</div></div></div>; })()}
    </section>
  );
}
