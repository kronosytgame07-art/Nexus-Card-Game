...
import { activateSupportCard, activateUnitEffect, aiBattlePhase, aiDrawPhase, aiEndPhase, aiMainPhase, declareAttack, evolveUnit, newGame, playCard } from './engine/engine';
...
function Combat() {
  ...
  const nextTurn = () => { setHandOpen(false); setPreviewCardId(null); if (match.activePlayer !== 'player' || match.winner) return; setSelectedAttacker(null); setPhase('draw'); triggerFx(null); const token = ++turnTokenRef.current; const beforeDraw = match; const afterDraw = aiDrawPhase(match); setAiDrawCount(Math.max(0, afterDraw.enemy.hand.length - beforeDraw.enemy.hand.length)); setAiTurnStage('draw'); updateMatch(afterDraw); window.setTimeout(() => { if (turnTokenRef.current !== token) return; setAiTurnStage('main'); const afterMain = aiMainPhase(afterDraw); updateMatch(afterMain); pulseFromDiff(afterDraw, afterMain); window.setTimeout(() => { if (turnTokenRef.current !== token) return; setAiTurnStage('battle'); const afterBattle = aiBattlePhase(afterMain); updateMatch(afterBattle); pulseFromDiff(afterMain, afterBattle); window.setTimeout(() => { if (turnTokenRef.current !== token) return; setAiTurnStage('end'); window.setTimeout(() => { if (turnTokenRef.current !== token) return; const afterEnd = aiEndPhase(afterBattle); updateMatch(afterEnd); pulseFromDiff(afterBattle, afterEnd); setAiTurnStage('idle'); if (!afterEnd.winner) setDrawStage('prompt'); }, 420); }, 700); }, 750); }, 620); };
