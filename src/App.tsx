...
const NAV_SECTIONS: { label: string; to: string; icon: string; matchPaths: string[] }[] = [
  { label: 'Jouer', to: '/', icon: '⚔', matchPaths: ['/', '/campagne', '/combat', '/replay'] },
  { label: 'Collection', to: '/collection', icon: '▣', matchPaths: ['/collection', '/decks'] },
  { label: 'Social', to: '/profil', icon: '👥', matchPaths: ['/profil', '/échanges', '/classement'] },
  { label: 'Boutique', to: '/boutique', icon: '🛒', matchPaths: ['/boutique'] },
  { label: 'Paramètres', to: '/paramètres', icon: '⚙', matchPaths: ['/paramètres'] },
];
...
function Shell({ children }: { children: React.ReactNode }) {
  ...
  const navItems = NAV_SECTIONS.map((section) => ({ section, active: isSectionActive(section, location.pathname) }));
  return <>
    <PortraitGate /><MusicManager />
    <aside className="app-sidebar">
      <h1><img className="brand-mark" src={LOGO_URL} alt="Nexus Arena" /> NEXUS <small>CARD ARENA</small></h1>
      {navItems.map(({ section, active }) => (
        <Link key={section.label} to={section.to} className={'nav-link' + (active ? ' active' : '')}>
          <span className="nav-link-icon">{section.icon}</span>{navTranslationKey(section.label) ? t(language, navTranslationKey(section.label)!) : section.label}
        </Link>
      ))}
    </aside>
    <main>{children}</main>
    <div className="hud-buttons">
      <Link to="/tutoriel" className="help-toggle" title="Tutoriel">﹖</Link>
      <MusicToggle /><FullscreenButton />
    </div>
  </>;
}
...
function CollectionHub() {
  ...
  return <>
    <div className="subtabs">
      <Link to="/collection" className={'subtab' + (tab === 'cartes' ? ' active' : '')}>{t(useGame.getState().language, 'common.cards')}</Link>
...
function DeckMenu({ onEdit }: { onEdit: (id: string) => void }) {
  ...
  return (
    <section>
      ...
      <p className="hint">Règles : {MAIN_DECK_MIN} à {MAIN_DECK_MAX} cartes · mélange libre des archétypes débloqués · 3 exemplaires max par carte, 1 seule Mythique · l'Évosphère (max {EVOSPHERE_MAX}) ne propose que les évolutions des unités réellement présentes dans le deck · uniquement les cartes possédées.</p>
    </section>
  );
}
...
type BattleFx = { type: 'summon'; side: 'player' | 'enemy'; instanceId?: string } | { type: 'attack'; side: 'player' | 'enemy'; instanceId: string; dx?: number; dy?: number } | { type: 'effect'; side: 'player' | 'enemy'; instanceId: string } | { type: 'evolution'; side: 'player' | 'enemy'; cardName: string } | null;
...
function FieldCard({ unit, isEnemy, taunted, selectable, selected, fx, onSelect, damagePulse, registerRef, hidden }: { unit: FieldUnit; isEnemy: boolean; taunted: boolean; selectable: boolean; selected: boolean; fx: BattleFx; onSelect?: (id: string) => void; damagePulse?: { key: string; amount: number }; registerRef?: (id: string, el: HTMLElement | null) => void; hidden?: boolean }) {
  ...
  return <motion.button ref={(el) => registerRef?.(unit.instanceId, el)} layout initial={{ scale: 0.15, opacity: 0, rotateY: 100 }} animate={isAttacking ? { scale: [1, 1.1, 1], x: [0, strikeX, 0], y: [0, strikeY, 0], opacity: 1, rotateY: 0, zIndex: 40 } : isSummoning ? { scale: [0.35, 1.18, 1], opacity: [0, 1, 1], rotateY: [90, -8, 0] } : isHit ? { scale: 1, opacity: 1, rotateY: 0, y: 0, x: [0, -7, 7, -5, 5, -2, 2, 0] } : { scale: 1, opacity: 1, rotateY: 0, y: 0, x: 0, zIndex: 1 }} transition={{ duration: isAttacking ? 0.62 : isHit ? 0.4 : 0.62, ease: 'easeOut' }} className={'field-card ' + card.rarity + (isEnemy && taunted && !unit.taunt ? ' not-targetable' : '') + (unit.taunt ? ' taunt' : '') + (unit.stunnedTurns > 0 ? ' stunned' : '') + (isHit ? ' hit-flash' : '') + (selected ? ' selected' : '') + (hidden ? ' evolving-hidden' : '') + (isEffect ? (isEnemy ? ' effect-active enemy-effect' : ' effect-active ally-effect') : '')} disabled={!selectable} onClick={() => onSelect?.(unit.instanceId)} data-card-id={card.id} data-instance-id={unit.instanceId} data-evolvable={Boolean(!isEnemy && card.waitTurns && card.evolvesTo && unit.turnsOnField >= card.waitTurns)} data-evolution-name={card.evolvesTo ? ALL_CARDS.find((entry) => entry.id === card.evolvesTo)?.name ?? '' : ''} data-card-text={card.text} data-card-cost={card.cost} data-card-rarity={card.rarity} data-card-faction={card.faction} data-wait-turns={card.waitTurns ?? ''} data-turns-on-field={unit.turnsOnField} data-effect-uses={unit.effectUsesThisTurn ?? 0} data-effect-max={effectMaxUses(card)} data-has-effect={Boolean(card.effect)}><img src={card.image} alt={card.name} onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = cardBack; }} /><span className="field-card-name">{card.name}</span><span className="field-card-level">NIV {card.level}</span>{card.evolvesTo && card.waitTurns && <span className={'field-card-evo' + (unit.turnsOnField >= card.waitTurns ? ' ready' : '')}>{unit.turnsOnField >= card.waitTurns ? 'ÉVO ✓' : 'ÉVO ' + unit.turnsOnField + '/' + card.waitTurns}</span>}<span className="field-card-atk">⚔ {unit.attack}</span><span className="field-card-hp">♥ {unit.health}</span><span className="field-card-tags">{card.flying && <em>VOL</em>}{card.ranged && <em>À DISTANCE</em>}{card.blitz && <em>BLITZ</em>}{card.attackDelayTurns && unit.turnsOnField < card.attackDelayTurns && <em>INERTIE {card.attackDelayTurns - unit.turnsOnField}</em>}{unit.taunt && <em>PROVOCATION</em>}{unit.stunnedTurns > 0 && <em>ÉTOURDI</em>}</span>{damagePulse && <motion.span key={damagePulse.key} className="dmg-float" initial={{ opacity: 1, y: 0, scale: 0.8 }} animate={{ opacity: [1, 1, 0], y: -46, scale: [0.8, 1.25, 1] }} transition={{ duration: 0.85, ease: 'easeOut' }}>-{damagePulse.amount}</motion.span>}</motion.button>;
}
...
function Zone({ title, units, isEnemy, taunted, selectable, selectedId, fx, onSelect, support, onActivateSupport, damagePulses, registerCardRef, evolvingId, placeableField, placeableSupport, onPlaceField, onPlaceSupport }: { title: string; units: FieldUnit[]; isEnemy: boolean; taunted: boolean; selectable: boolean; selectedId?: string | null; fx: BattleFx; onSelect?: (id: string) => void; support: SupportCard[]; onActivateSupport?: (instanceId: string) => void; damagePulses: Record<string, { key: string; amount: number }>; registerCardRef?: (id: string, el: HTMLElement | null) => void; evolvingId?: string | null; placeableField?: boolean; placeableSupport?: boolean; onPlaceField?: (slot: number) => void; onPlaceSupport?: (slot: number) => void }) {
  return <div className={'zone-wrap ' + (isEnemy ? 'enemy-zone' : 'player-zone')}><b>{title}</b><div className="board">{Array.from({ length: 3 }, (_, index) => { const unit = units.find((u) => u.slot === index); return unit ? <FieldCard key={unit.instanceId} unit={unit} isEnemy={isEnemy} taunted={taunted} selectable={selectable} selected={selectedId === unit.instanceId} fx={fx} onSelect={onSelect} damagePulse={damagePulses[unit.instanceId]} registerRef={registerCardRef} hidden={evolvingId === unit.instanceId} /> : <button type="button" className={'field-slot' + (placeableField ? ' placeable' : '')} key={`slot-${index}`} disabled={!placeableField} onClick={() => onPlaceField?.(index)}>◇</button>; })}</div><div className="support-row" aria-label="Zone de soutien">{Array.from({ length: 5 }, (_, index) => { const item = support.find((s) => s.slot === index); if (!item) return <button type="button" key={`sup-empty-${index}`} className={'support-slot' + (placeableSupport ? ' placeable' : '')} disabled={!placeableSupport} onClick={() => onPlaceSupport?.(index)}>◇</button>; const def = getCard(item.cardId); return <button key={item.instanceId} className="support-card" style={{ backgroundImage: `url(${CARD_BACK_URL})` }} disabled={isEnemy || !onActivateSupport} title={isEnemy ? 'Sort adverse posé face cachée' : `${def.name} — clique pour voir l'aperçu`} onClick={() => onActivateSupport?.(item.instanceId)} />; })}</div><small>SOUTIEN {support.length}/5</small></div>;
}

function Combat() {
  const s = useGame(); const location = useLocation(); const go = useNavigate();
  const arenaBgPaused = s.animationMode === 'off' || s.batterySaver;
  const routeState = location.state as { chapterId?: number; onlineMode?: 'classic' | 'ranked' } | null;
  ...
  const onlineAiDifficulty = onlineMode === 'ranked'
    ? aiDifficultyForRating(s.rankedRating ?? DEFAULT_RANKED_RATING)
    : onlineMode === 'classic'
      ? AI_DIFFICULTIES[Math.floor(Math.random() * AI_DIFFICULTIES.length)]
      : undefined;
  const aiDifficulty = chapter ? chapter.aiDifficulty : onlineAiDifficulty ?? 'novice'; const lifeBonus = chapter ? chapter.enemyLifeBonus : 0; const reward = chapter ? chapter.reward : 35;
  ...
  const playOpeningDraw = () => { setDrawStage('reveal'); vfxRef.current?.spawnBurst(window.innerWidth / 2, window.innerHeight / 2, 'draw'); window.setTimeout(() => { setDrawStage('idle'); setPhase('main'); }, 900); };
  ...
  useLayoutEffect(() => { playOpeningDraw(); }, []);
  useEffect(() => { if (!match.winner || reported) return; const won = match.winner === 'player'; s.record(won); if (won) { s.addGold(reward); s.addGems(WIN_GEMS_REWARD); if (chapter) s.completeChapter(chapter.id); } if (onlineMode === 'ranked') s.recordRanked(won); setReported(true); }, [match.winner, reported, reward, chapter, onlineMode, s]);
  ...
  const confirmPlay = (id: string, slotIndex?: number) => { if (match.activePlayer !== 'player' || match.winner) return; if (phase !== 'main') { showHint('Passe en Main Phase pour jouer une carte.'); setPreviewCardId(null); setPlacingCard(null); return; } const card = ALL_CARDS.find((entry) => entry.id === id); const before = match; const next = playCard(match, 'player', id, slotIndex); setPreviewCardId(null); setPlacingCard(null); if (!stateChanged(match, next)) return; updateMatch(next); setHandOpen(false); pulseFromDiff(before, next); if (card?.type === 'unit') { const newest = next.player.field[next.player.field.length - 1]; triggerFx({ type: 'summon', side: 'player', instanceId: newest?.instanceId }, 720); if (newest) { const id = newest.instanceId; requestAnimationFrame(() => requestAnimationFrame(() => { const el = cardRefs.current[id]; if (el) { const r = el.getBoundingClientRect(); vfxRef.current?.spawnBurst(r.left + r.width / 2, r.top + r.height / 2, 'summon'); } })); } if (newest && effectMaxUses(card) > 0) { const promptId = newest.instanceId; window.setTimeout(() => setEffectPrompt(promptId), 760); } } };
  const beginPlacement = (id: string) => { const def = getCard(id); if (def.type === 'unit' && match.player.field.length >= MAX_FIELD_UNITS) { showHint('Ton terrain est plein.'); setPreviewCardId(null); return; } if (def.type === 'spell' && match.player.support.length >= MAX_SUPPORT) { showHint('Ta zone de Soutien est pleine.'); setPreviewCardId(null); return; } setPreviewCardId(null); setPlacingCard({ id, type: def.type }); showHint(def.type === 'unit' ? 'Choisis un emplacement sur ton terrain.' : 'Choisis un emplacement dans ta zone de Soutien.', 4000); };
  const placeCard = (slotIndex: number) => { if (!placingCard) return; confirmPlay(placingCard.id, slotIndex); };
  const selectAttacker = (id: string) => { setHandOpen(false); setPreviewCardId(null); setInspectedEnemyId(null); setInspectedUnit((current) => current === id ? null : id); if (match.activePlayer !== 'player' || match.winner || attackLockRef.current) return; if (phase !== 'battle') { showHint('Passe en Battle Phase pour attaquer.'); return; } const unit = match.player.field.find((entry) => entry.instanceId === id); if (!unit || !unit.canAttack || unit.stunnedTurns > 0) return; setSelectedAttacker((current) => current === id ? null : id); };
  const inspectEnemyUnit = (id: string) => { if (selectedAttacker && phase === 'battle' && !attackLockRef.current) { attackUnit(id); return; } setInspectedUnit(null); setInspectedEnemyId((current) => current === id ? null : id); };
  ...
  const attackUnit = (targetId: string) => { setHandOpen(false); setPreviewCardId(null); if (!selectedAttacker || phase !== 'battle' || attackLockRef.current) return; resolveStrike(selectedAttacker, targetId); setSelectedAttacker(null); setInspectedEnemyId(null); };
...
function SocialHub() {
  ...
  return <>
    <div className="subtabs">
      <Link to="/profil" className={'subtab' + (tab === 'profil' ? ' active' : '')}>{t(useGame.getState().language, 'common.profile')}</Link>
    ...
    {tab === 'profil' && <Profile />}
    {tab === 'echanges' && <Suspense fallback={<section><h2>Échanges</h2><p className="hint">Chargement…</p></section>}><Trades /></Suspense>}
...
function Options() {
  ...
  return <section><h2>{t(s.language, 'settings.title')}</h2><div className="options-grid settings-grid">
    <article className="options-card settings-card"><div className="options-card-head"><span className="menu-card-icon teal">🔊</span><b>{t(s.language, 'settings.audio')}</b></div>
    ...
    <article className="options-card settings-card"><div className="options-card-head"><span className="menu-card-icon violet">🖥</span><b>{t(s.language, 'settings.display')}</b></div>
    ...
    <article className="options-card settings-card"><div className="options-card-head"><span className="menu-card-icon gold">🌐</span><b>{t(s.language, 'settings.language')}</b></div>
      <div className="option-row"><div className="option-row-text"><b>{t(s.language, 'settings.interfaceLanguage')}</b></div><select value={s.language} onChange={(e) => s.setLanguage(e.target.value as Language)}>{(Object.keys(LANGUAGE_LABELS) as Language[]).map((language) => <option key={language} value={language}>{LANGUAGE_LABELS[language]}</option>)}</select></div>
...
const BOOSTERS: { id: Faction; name: string; price: number; blurb: string; available?: boolean }[] = [
  ...
  { id: 'Gobelin', name: 'Booster Gobelin', price: 150, blurb: 'Archétype essaim ultra-agressif : petites unités fragiles avec Blitz et invocations spéciales depuis le deck.' },
];
        ...
        {purchasableAvatarCards().map((card) => {
          ...
          return (
            <button key={card.id} type="button" className={'avatar-choice' + (owned ? ' active' : '')} disabled={owned || s.gems < AVATAR_PRICE_GEMS} onClick={() => s.purchaseAvatar(card.id)}>
              <img src={card.image} alt={card.name} onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = cardBack; }} />
              <span>{card.name}</span>
              <span className="hint">{owned ? '✓ Débloqué' : `💎 ${AVATAR_PRICE_GEMS}`}</span>
            </button>
          );
        })}
      </div>
        ...
        {TERRAINS.map((terrain) => {
          ...
          return (
            <article key={terrain.id} className={'terrain-card' + (active ? ' active' : '')}>
              ...
              {owned ? (
                <button className={active ? 'primary' : 'secondary'} disabled={active} onClick={() => s.selectTerrain(terrain.id)}>{active ? '✓ Actif' : 'Sélectionner'}</button>
              ) : (
                <button className="primary" disabled={s.gems < TERRAIN_PRICE_GEMS} onClick={() => s.purchaseTerrain(terrain.id)}>💎 {TERRAIN_PRICE_GEMS}</button>
              )}
            </article>
          );
        })}
...
function Multiplayer() {
  ...
  const cancelSearch = () => { if (searchTimer.current) window.clearTimeout(searchTimer.current); setSearching(null); };
  ...
  return <section className="multiplayer-page"><h2>Multijoueur</h2><p className="hint">Aucun serveur de matchmaking temps réel n'est encore branché : si aucun joueur n'est trouvé après une courte recherche, un bot prend le relais — niveau aléatoire en Classique, niveau équivalent à ton rang en Classé.</p><div className="multiplayer-modes"><article className="multiplayer-mode"><span className="menu-card-icon teal">⚔</span><h3>Classique</h3><p>Duels sans impact sur le rang. Idéal pour tester un deck hybride, apprendre un nouvel archétype ou jouer entre amis.</p><button className="primary" disabled={searching !== null} onClick={() => startSearch('classic')}>{searching === 'classic' ? 'RECHERCHE…' : 'RECHERCHER UN ADVERSAIRE'}</button><small>Repli automatique sur un bot de niveau aléatoire si personne n'est trouvé</small></article><article className="multiplayer-mode ranked"><span className="menu-card-icon gold">✦</span><h3>Classé</h3><p>Les victoires et défaites font évoluer ton statut dans le Nexus.</p><div className="rank-card"><b>{formatRank(rank)}</b><span>{currentRating} points · {rankedWins}V / {rankedLosses}D</span>{next && <small>Prochain palier à {next.minRating} points</small>}</div><button className="primary" disabled={searching !== null} onClick={() => startSearch('ranked')}>{searching === 'ranked' ? 'RECHERCHE…' : 'LANCER UN DUEL CLASSÉ'}</button><small>Repli automatique sur un bot de ton rang si personne n'est trouvé</small></article></div><h3>Rangs du Nexus</h3><div className="rank-ladder">{RANKED_LADDER.filter((entry, index, arr) => index === 0 || arr[index - 1].tier !== entry.tier).map((entry) => <div key={entry.tier}><b>{entry.tier}</b><span>{entry.tier === 'Légende du Nexus' ? String(entry.minRating) + '+' : String(entry.minRating) + '–' + String(RANKED_LADDER.filter((r) => r.tier === entry.tier).at(-1)?.maxRating ?? '')}</span></div>)}</div>{searching && <div className="matchmaking-overlay" role="dialog" aria-modal="true"><div className="matchmaking-card"><span className="matchmaking-spinner" aria-hidden="true" /><b>Recherche d'un adversaire{searching === 'ranked' ? ' classé' : ''}…</b><small>{botDifficultyHint} si aucun joueur n'est disponible</small><button className="secondary" onClick={cancelSearch}>Annuler</button></div></div>}</section>;
}
...
export default function App() { const factionChosen = useGame((s) => s.factionChosen); const hasLegacyDeck = useGame((s) => s.deck.length > 0); if (!factionChosen && !hasLegacyDeck) return <><AudioDirector /><SecretDragonReward /><FactionOnboarding /></>; return <><DisplaySettingsBridge /><AudioDirector /><SecretDragonReward /><Suspense fallback={null}><GoogleSignInGate /></Suspense><Shell><Routes><Route path="/" element={<Home />} /><Route path="/campagne" element={<NarrativeCampaign />} /><Route path="/chroniques" element={<CodexPanel />} /><Route path="/collection" element={<CollectionHub />} /><Route path="/decks" element={<CollectionHub />} /><Route path="/échanges" element={<SocialHub />} /><Route path="/profil" element={<SocialHub />} /><Route path="/replay/:id" element={<Replay />} /><Route path="/combat" element={<Combat />} /><Route path="/multijoueur" element={<Multiplayer />} /><Route path="/paramètres" element={<Options />} /><Route path="/classement" element={<SocialHub />} /><Route path="/boutique" element={<Shop />} /><Route path="/tutoriel" element={<Tutorial />} /></Routes></Shell></>; }
