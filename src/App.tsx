...
function FieldCard({ unit, isEnemy, taunted, selectable, selected, fx, onSelect, damagePulse, registerRef }: { unit: FieldUnit; isEnemy: boolean; taunted: boolean; selectable: boolean; selected: boolean; fx: BattleFx; onSelect?: (id: string) => void; damagePulse?: { key: string; amount: number }; registerRef?: (id: string, el: HTMLElement | null) => void }) {
...
function Combat() {
  ...
  const [fx, setFx] = useState<BattleFx>(null); const [pileOpen, setPileOpen] = useState<null | 'grave' | 'evo'>(null); const fxTimer = useRef<number | null>(null); const turnTokenRef = useRef(0); const cardRefs = useRef<Record<string, HTMLElement | null>>({}); const heroRefs = useRef<{ player: HTMLElement | null; enemy: HTMLElement | null }>({ player: null, enemy: null }); const attackLockRef = useRef(false); const replaySnapshots = useRef<GameState[]>([]); const stripForReplay = (state: GameState): GameState => ({ ...state, log: [] }); const updateMatch = (next: GameState) => { setMatch(next); replaySnapshots.current.push(stripForReplay(next)); }; useEffect(() => { replaySnapshots.current = [stripForReplay(match)]; }, []); const [replaySaved, setReplaySaved] = useState(false); const registerCardRef = (id: string, el: HTMLElement | null) => { cardRefs.current[id] = el; }; const measureStrike = (attackerId: string, targetId: string | null) => { const attackerEl = cardRefs.current[attackerId]; const targetEl = targetId ? cardRefs.current[targetId] : heroRefs.current.enemy; if (!attackerEl || !targetEl) return { dx: 0, dy: 0 }; const a = attackerEl.getBoundingClientRect(); const t = targetEl.getBoundingClientRect(); return { dx: (t.left + t.width / 2) - (a.left + a.width / 2), dy: (t.top + t.height / 2) - (a.top + a.height / 2) }; };
  ...
  const resolveStrike = (attackerId: string, targetId: string | null) => { const { dx, dy } = measureStrike(attackerId, targetId); attackLockRef.current = true; triggerFx({ type: 'attack', side: 'player', instanceId: attackerId, dx, dy }, 640); window.setTimeout(() => { const before = match; const next = declareAttack(before, 'player', attackerId, targetId); updateMatch(next); pulseFromDiff(before, next); attackLockRef.current = false; }, 300); };
  ...
  const nextTurn = () => { setHandOpen(false); setPreviewCardId(null); if (match.activePlayer !== 'player' || match.winner) return; setSelectedAttacker(null); setPhase('draw'); const before = match; triggerFx(null); const afterPlayerEnd = endTurn(match); setAiDrawCount(Math.max(0, afterPlayerEnd.enemy.hand.length - match.enemy.hand.length)); const token = ++turnTokenRef.current; setAiTurnStage('draw'); window.setTimeout(() => { if (turnTokenRef.current !== token) return; setAiTurnStage('main'); window.setTimeout(() => { if (turnTokenRef.current !== token) return; setAiTurnStage('battle'); window.setTimeout(() => { if (turnTokenRef.current !== token) return; setAiTurnStage('end'); window.setTimeout(() => { if (turnTokenRef.current !== token) return; updateMatch(afterPlayerEnd); pulseFromDiff(before, afterPlayerEnd); setAiTurnStage('idle'); if (!afterPlayerEnd.winner) setDrawStage('prompt'); }, 420); }, 520); }, 560); }, 700); };
  const confirmDraw = () => { setDrawStage('reveal'); window.setTimeout(() => { setDrawStage('idle'); setPhase('main'); }, 900); };
...
function ReplayUnit({ unit }: { unit: FieldUnit }) {
  const card = ALL_CARDS.find((c) => c.id === unit.cardId);
  if (!card) return null;
  return <div className="replay-unit"><img src={card.image} alt={card.name} onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = cardBack; }} /><span className="replay-unit-name">{card.name}</span><span className="replay-unit-atk">⚔ {unit.attack}</span><span className="replay-unit-hp">♥ {unit.health}</span></div>;
}
...
function Replay() {
  ...
  return <section className="replay-view">
    <div className="replay-head">
      <button className="secondary" onClick={() => go('/profil')}>← Profil</button>
      <div><h2>{replay.label}</h2><p className={'hint replay-result-tag ' + replay.result}>{replay.result === 'win' ? '🏆 Victoire' : '💀 Défaite'} · vs {replay.opponentFaction} · {replay.turns} tours · {new Date(replay.date).toLocaleDateString()}</p></div>
    </div>
    <div className="replay-board">
      <div className="replay-side">
        <div className="replay-side-head"><b>Adversaire</b><small>{replay.opponentFaction}</small><span className="life">♥ {snap.enemy.life}</span><span className="mana">◆ {snap.enemy.mana}/{snap.enemy.maxMana}</span></div>
        <div className="replay-hand">{snap.enemy.hand.map((_, i) => <span key={i} className="replay-card-back" style={{ backgroundImage: `url(${CARD_BACK_URL})` }} />)}</div>
        <div className="replay-row">{Array.from({ length: 3 }, (_, i) => { const u = snap.enemy.field[i]; return u ? <ReplayUnit key={u.instanceId} unit={u} /> : <div className="replay-slot" key={`e${i}`}>◇</div>; })}</div>
        <div className="replay-row support">{Array.from({ length: 5 }, (_, i) => snap.enemy.support[i] ? <span key={i} className="replay-support" style={{ backgroundImage: `url(${CARD_BACK_URL})` }} /> : <div className="replay-slot small" key={`es${i}`}>◇</div>)}</div>
      </div>
      <div className="replay-meta">Tour {snap.turn} · {snap.activePlayer === 'player' ? 'Tour du joueur' : "Tour de l'adversaire"}</div>
      <div className="replay-side">
        <div className="replay-row support">{Array.from({ length: 5 }, (_, i) => snap.player.support[i] ? <span key={i} className="replay-support" style={{ backgroundImage: `url(${CARD_BACK_URL})` }} /> : <div className="replay-slot small" key={`ps${i}`}>◇</div>)}</div>
        <div className="replay-row">{Array.from({ length: 3 }, (_, i) => { const u = snap.player.field[i]; return u ? <ReplayUnit key={u.instanceId} unit={u} /> : <div className="replay-slot" key={`p${i}`}>◇</div>; })}</div>
        <div className="replay-hand">{snap.player.hand.map((cardId, i) => { const c = getCard(cardId); return <img key={i} className="replay-hand-card" src={c.image} alt={c.name} onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = cardBack; }} />; })}</div>
        <div className="replay-side-head"><b>{s.playerName}</b><span className="life">♥ {snap.player.life}</span><span className="mana">◆ {snap.player.mana}/{snap.player.maxMana}</span></div>
      </div>
    </div>
    <div className="replay-controls">
      <button className="secondary" onClick={() => { setPlaying(false); setStep(0); }} disabled={step === 0}>⏮</button>
      <button className="secondary" onClick={() => { setPlaying(false); setStep((n) => Math.max(0, n - 1)); }} disabled={step === 0}>◀ Précédent</button>
      <span className="replay-step">{step + 1}/{total}</span>
      <button className="secondary" onClick={() => { setPlaying(false); setStep((n) => Math.min(total - 1, n + 1)); }} disabled={step === total - 1}>Suivant ▶</button>
