...
function Combat() {
  ...
  const play = (id: string) => { if (match.activePlayer !== 'player' || match.winner) return; if (!handOpen) { setHandOpen(true); return; } const card = ALL_CARDS.find((entry) => entry.id === id); const before = match; const next = playCard(match, 'player', id); if (next === match) return; setMatch(next); setHandOpen(false); pulseFromDiff(before, next); if (card?.type === 'unit') { const newest = next.player.field[next.player.field.length - 1]; triggerFx({ type: 'summon', side: 'player', instanceId: newest?.instanceId }, 720); } };
  const selectAttacker = (id: string) => { setHandOpen(false); setInspectedUnit((current) => current === id ? null : id); if (match.activePlayer !== 'player' || match.winner) return; const unit = match.player.field.find((entry) => entry.instanceId === id); if (!unit || !unit.canAttack || unit.stunnedTurns > 0) return; setSelectedAttacker((current) => current === id ? null : id); };
  const attackUnit = (targetId: string) => { setHandOpen(false); if (!selectedAttacker) return; const before = match; triggerFx({ type: 'attack', side: 'player', instanceId: selectedAttacker }, 520); const next = declareAttack(match, 'player', selectedAttacker, targetId); setMatch(next); pulseFromDiff(before, next); setSelectedAttacker(null); };
  const attackHero = () => { setHandOpen(false); if (!selectedAttacker) return; const before = match; triggerFx({ type: 'attack', side: 'player', instanceId: selectedAttacker }, 520); const next = declareAttack(match, 'player', selectedAttacker, null); setMatch(next); pulseFromDiff(before, next); setSelectedAttacker(null); };
  const evolve = (instanceId: string) => { setHandOpen(false); const beforeUnit = match.player.field.find((entry) => entry.instanceId === instanceId); if (!beforeUnit) return; const beforeCard = ALL_CARDS.find((entry) => entry.id === beforeUnit.cardId); const evolvedCard = beforeCard?.evolvesTo ? ALL_CARDS.find((entry) => entry.id === beforeCard.evolvesTo) : undefined; const next = evolveUnit(match, 'player', instanceId); if (next === match) return; setMatch(next); triggerFx({ type: 'evolution', side: 'player', cardName: evolvedCard?.name ?? 'Évolution' }, 1000); };
  const activateEffect = (instanceId: string) => { setHandOpen(false); const before = match; const unit = before.player.field.find((entry) => entry.instanceId === instanceId); if (!unit) return; const card = ALL_CARDS.find((entry) => entry.id === unit.cardId); const next = activateUnitEffect(before, 'player', instanceId); if (next === before) { setEffectHint('Cet effet ne peut pas être activé maintenant.'); window.setTimeout(() => setEffectHint(''), 2000); return; } setMatch(next); pulseFromDiff(before, next); setEffectHint(card ? `Effet de ${card.name} activé !` : 'Effet activé !'); window.setTimeout(() => setEffectHint(''), 2000); };
  const activateSupport = (instanceId: string) => { setHandOpen(false); const before = match; const item = before.player.support.find((entry) => entry.instanceId === instanceId); if (!item) return; const card = getCard(item.cardId); const next = activateSupportCard(before, 'player', instanceId); if (next === before) { setEffectHint(`${card.name} ne peut pas être activé maintenant.`); window.setTimeout(() => setEffectHint(''), 2200); return; } setMatch(next); pulseFromDiff(before, next); setEffectHint(`${card.name} activé !`); window.setTimeout(() => setEffectHint(''), 2000); };
  const nextTurn = () => { setHandOpen(false); if (match.activePlayer !== 'player' || match.winner) return; setSelectedAttacker(null); setPhase('main'); const before = match; triggerFx(null); const afterPlayerEnd = endTurn(match); setMatch(afterPlayerEnd); pulseFromDiff(before, afterPlayerEnd); };
  const restart = () => { setMatch(startMatch()); setReported(false); setSelectedAttacker(null); setInspectedHandCard(null); setEffectHint(''); setInspectedUnit(null); setUnitPulses({}); setHeroPulses({}); setHandOpen(false); setPhase('main'); };
  useEffect(() => { const onKey = (event: KeyboardEvent) => { if (event.key !== 'Escape') return; setInspectedUnit(null); setSelectedAttacker(null); setInspectedHandCard(null); }; document.addEventListener('keydown', onKey); return () => document.removeEventListener('keydown', onKey); }, []);
