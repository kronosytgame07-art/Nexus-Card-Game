...
function Combat() {
  ...
  const nextTurn = () => { setHandOpen(false); if (match.activePlayer !== 'player' || match.winner) return; setSelectedAttacker(null); setPhase('draw'); const before = match; triggerFx(null); const afterPlayerEnd = endTurn(match); setMatch(afterPlayerEnd); pulseFromDiff(before, afterPlayerEnd); };
  ...
  const restart = () => { setMatch(startMatch()); setReported(false); setSelectedAttacker(null); setInspectedHandCard(null); setEffectHint(''); setInspectedUnit(null); setUnitPulses({}); setHeroPulses({}); setHandOpen(false); setPhase('draw'); };
