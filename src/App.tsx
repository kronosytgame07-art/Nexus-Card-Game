...
function Combat() {
  ...
  const [fx, setFx] = useState<BattleFx>(null); const [pileOpen, setPileOpen] = useState<null | 'grave' | 'evo'>(null); const fxTimer = useRef<number | null>(null); const turnTokenRef = useRef(0); const cardRefs = useRef<Record<string, HTMLElement | null>>({}); const heroRefs = useRef<{ player: HTMLElement | null; enemy: HTMLElement | null }>({ player: null, enemy: null }); const attackLockRef = useRef(false); const registerCardRef = (id: string, el: HTMLElement | null) => { cardRefs.current[id] = el; }; const measureStrike = (attackerId: string, targetId: string | null) => { const attackerEl = cardRefs.current[attackerId]; const targetEl = targetId ? cardRefs.current[targetId] : heroRefs.current.enemy; if (!attackerEl || !targetEl) return { dx: 0, dy: 0 }; const a = attackerEl.getBoundingClientRect(); const t = targetEl.getBoundingClientRect(); return { dx: (t.left + t.width / 2) - (a.left + a.width / 2), dy: (t.top + t.height / 2) - (a.top + a.height / 2) }; };
...
function Options() {
  ...
  return <section><h2>Options</h2><div className="options-grid">
    ...
    <article className="options-card danger"><b>Zone dangereuse</b><p className="hint">Efface définitivement toute ta progression : decks, cartes possédées, niveau, XP, victoires/défaites, campagne et pseudo. Cette action est irréversible.</p><button className="secondary danger" onClick={() => setResetConfirmOpen(true)}>🗑 Recommencer le jeu à zéro</button></article>
  </div>{resetConfirmOpen && <div className="reset-confirm-overlay" role="dialog" aria-modal="true" onClick={closeResetConfirm}><div className="reset-confirm" onClick={(e) => e.stopPropagation()}><b>⚠ Réinitialisation totale</b><p>Tu es sur le point d'effacer <u>définitivement</u> toute ta progression : decks, cartes, niveau, XP, victoires, défaites et campagne.</p><p>Cette action est <b>irréversible</b> — il n'existe aucun moyen de récupérer ces données ensuite. Assure-toi d'être sûr à 100 % avant de continuer.</p><p className="reset-confirm-hint">Pour confirmer, tape <b>{RESET_CONFIRM_WORD}</b> ci-dessous :</p><input value={resetConfirmText} onChange={(e) => setResetConfirmText(e.target.value)} placeholder={RESET_CONFIRM_WORD} autoFocus /><div className="reset-confirm-actions"><button className="secondary" onClick={closeResetConfirm}>Annuler</button><button className="secondary danger" disabled={resetConfirmText.trim().toUpperCase() !== RESET_CONFIRM_WORD} onClick={doHardReset}>Tout supprimer définitivement</button></div></div></div>}</section>;
}
