...
function Combat() {
  ...
  const selectAttacker = (id: string) => { setHandOpen(false); if (match.activePlayer !== 'player' || match.winner) return; const unit = match.player.field.find((entry) => entry.instanceId === id); if (!unit || !unit.canAttack || unit.stunnedTurns > 0) return; setSelectedAttacker((current) => current === id ? null : id); };
  ...
  const nextTurn = () => { setHandOpen(false); if (match.activePlayer !== 'player' || match.winner) return; setSelectedAttacker(null); const before = match; triggerFx(null); const afterPlayerEnd = endTurn(match); setMatch(afterPlayerEnd); pulseFromDiff(before, afterPlayerEnd); };
  const restart = () => { setMatch(startMatch()); setReported(false); setSelectedAttacker(null); setInspectedHandCard(null); setEffectHint(''); setInspectedUnit(null); setUnitPulses({}); setHeroPulses({}); setHandOpen(false); };
