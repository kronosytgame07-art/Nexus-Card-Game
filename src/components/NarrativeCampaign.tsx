...
export function NarrativeCampaign(){
 ...
 return <section className="narrative-campaign"><header><small>CHAPITRE I</small><h2>Les Éclats du Serment</h2><p>Le Nexus s’est brisé. Dans les fragments, quelque chose se souvient de toi.</p><button className="secondary" onClick={()=>setCodex(true)}>Chroniques d’Elyndra</button></header><div className="chapter-list">{CHAPTER_ONE.map((duel,index)=>{const locked=index>0&&!completed.includes(CHAPTER_ONE[index-1].id);const done=completed.includes(duel.id);return <article key={duel.id} className={'chapter '+(locked?'locked':'')}><span className="number">0{index+1}</span><div className="chapter-body"><b>{duel.title}</b><small>{done?'Scène et duel terminés':locked?'Poursuis ton enquête':'Cinématique BD · Duel '+duel.faction}</small></div><button disabled={locked} onClick={()=>setScene(index)}>{done?'Rejouer':'Commencer'}</button></article>})}</div>{active&&<StoryDialog sceneId={`chapter-1-${active.id}`} lines={active.lines} playerName={playerName} onClose={()=>setScene(null)} onComplete={()=>go('/combat',{state:{chapterId:active.id,narrativeDuelId:active.id}})}/>}</section>;
}
